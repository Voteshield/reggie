...
from veggie.utilities import date_from_str
import json
...
class Preprocessor():
    ...
    def unpack_files(self, file_obj=None, compression="unzip"):
        if file_obj is None:
    ...
    def unzip_decompress(self, file_name):
        ...
        if self.state == "new_york":
            ...
            logging.info("decompressing unzip {} into {}".format(file_name,
                                                                 file_names))
            file_objs = [{"name": name, "obj": BytesIO(zip_file.read(name))}
                         for name in file_names]
        return file_objs
